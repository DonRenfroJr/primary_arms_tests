...
Replace 4 <your_common_name> and 3 <your_common_name> items wit your values.
...
def run(context):
    ...
    hornady_btn = element_exists(context, 'btn_300_blk_hornady')
    
    while not hornady_btn and attempts <= MAX_SWIPES:
        attempts += 1
        context.perform_gesture('swipe_up', '')
        hornady_btn = element_exists(context, 'btn_300_blk_hornady')

    context.verify(['btn_300_blk_hornady'])
